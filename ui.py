...
# --- 토큰 ---------------------------------------------------
# ⚠️ 2026-09-22 전면 개편 — 다크, 액센트 둘.
#    예전 포인트 파랑(#1A56DB)+금색(#C8963E) 조합은 폐기했다.
#    파랑·회색·금색이 한 화면에서 서로 주인공을 하려 해서 산만했고,
#    금색은 어떻게 써도 낡아 보였다.
#    지금은 역할로 나눈다 — 아이리스(보라)=제품이 하는 일,
#    민트=당신이 벌 돈. 상태색은 그 둘과 따로 둔다.
#    여기 열 줄만 바꾸면 ui.py가 뿜는 인라인 색이 전부 따라온다.
#    화면 구조·글자는 web/static/kh.css 하나가 담당한다.
INK = "#3A2320"        # 본문 — 붉은 기 도는 먹색 (2026-09-23: 31번 Peach Fuzz)
MUTED = "#A07F76"      # 보조 텍스트 — 따뜻한 회갈색
LINE = "#F7D9CD"       # 경계선 (헤어라인)
BASE = "#FFF1EA"       # 배경 — 순백보다 반 톤 낮다 (카드가 떠 보이게)
SURFACE = "#FFFFFF"    # 카드·패널 — 바닥보다 밝다
DEEP = "#F2604E"       # 구조 액센트 — 코랄 (제품·차트선·버튼)
GOLD = "#A8690A"       # 기회 신호 (골든타임) — 앰버 (흰 글자 4.5:1 확보)
GOOD = "#0A9D6B"       # 좋음 · 돈 (에메랄드)
WARN = "#7A86A8"       # 보통 — 중립은 회색이 정직하다
BAD = "#B42C45"        # 나쁨 — 액센트 코랄과 갈리게 한 단 깊은 크림슨
...
/* 제목이 길면 …로 자르는 대신 손가락으로 밀어서 끝까지 본다 (2026-08-28) */
.serp-title {{
overflow-x: auto; text-overflow: clip;
-webkit-overflow-scrolling: touch; scrollbar-width: none;
}}
.serp-title::-webkit-scrollbar {{ display: none; }}
...
/* ⚠️ 청약처럼 긴 이름이 붙박이 첫 열을 화면 폭만큼 키워
   옆 칸이 하나도 안 보이던 문제 (2026-08-28, 주간 캘린더).
   좁은 화면에서만 첫 열을 화면의 45%까지로 묶고,
   넘치면 단어 단위(keep-all)로 접어 두 줄로 보여준다.
   행 높이가 조금 어긋나는 것보다 표가 안 보이는 게 훨씬 나쁘다. */
.kh-t .kh-key {{
  padding-right: 14px;
  width: 45vw; min-width: 45vw; max-width: 45vw;
  white-space: normal;
  word-break: keep-all;
  overflow-wrap: anywhere;   /* 띄어쓰기 없는 긴 한 단어면 글자에서라도 끊는다 */
  line-height: 1.35;
}}
/* 그래도 넘치면 2줄에서 …으로 자른다 (전체 이름은 title로 남아 있다) */
.kh-t .kh-key .kh-kw {{
  display: -webkit-inline-box;
  -webkit-box-orient: vertical;
  -webkit-line-clamp: 2;
  overflow: hidden;
  /* 순번 숫자 옆에 나란히 앉게 폭을 그만큼 비워둔다
     (100%로 하면 상자가 순번 아랫줄로 떨어진다) */
...
[role="tablist"]::-webkit-scrollbar {{ display: none; }}
[role="tab"] {{
min-height: 44px !important; padding: 12px 18px !important;
background: transparent !important;
...
[role="tab"] *, [role="tab"] p, [role="tab"] div, [role="tab"] span {{
font-size: .98rem !important; font-weight: 600 !important; color: inherit !important;
}}
[role="tab"]:hover {{
...
[role="tab"][aria-selected="true"] {{
background: transparent !important; color: {DEEP} !important;
box-shadow: none !important;
border-bottom: 3px solid {DEEP} !important;
}}
[role="tab"][aria-selected="true"] *,
[role="tab"][aria-selected="true"] p,
[role="tab"][aria-selected="true"] div,
[role="tab"][aria-selected="true"] span {{
color: {DEEP} !important; font-weight: 800 !important;
}}
[role="tab"][aria-selected="true"]:hover {{
...
.mast-name {{
/* 2026-08-28: 브랜드가 너무 소심하다는 피드백 — 로고·이름 확대 */
font-size: 1.32rem; font-weight: 800; color: {INK};
...
[role="tablist"] {{ padding-left: 196px !important; height: 58px !important; align-items: center !important; }}
