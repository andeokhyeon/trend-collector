...
GRADE_TINT = {
    ...
    # 돈 판정 (2026-09-23, naver_api.calc_money_verdict) — 좋음 초록 / 애매 앰버 / 나쁨 붉은색
    '돈 되는 자리': ('#DCEFE9', '#175247'),
    '작게 해볼 만함': ('#FBF2E1', '#8A6420'), '비싸지만 좁음': ('#FBF2E1', '#8A6420'),
    '방문자용': ('#F1F1EC', '#5F5A52'), '단가 없음': ('#F1F1EC', '#6B7280'),
    '피하세요': ('#FAEAE5', '#9E3E28'),
    # 광고 경쟁 (검색광고 compIdx)
    '중간': ('#FBF2E1', '#8A6420'),
}

TINT_COLS = {'검색량', '경쟁률', '진단', '판단', '판정', '등급', '종류', '경쟁', '광고 경쟁', '언제 쓸까'}
...
# ⚠️ '클릭단가(원)'가 맨 앞이다 — 이 제품이 재는 것이 결국 돈이라
#    열이 잘리는 모바일에서도 단가는 남아야 한다 (2026-09-22).
PRIORITY_COLS = ('클릭단가(원)', '황금 점수', '광고단가', '진단', '판단',
                 '기회 점수', '내 승산', '경쟁률', '검색량', '경쟁')
